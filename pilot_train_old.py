import pilot_data_old as pilot_data
...
class Configuration:
    ...
    feature_dimension = -1
    output_size = -1
    sample = 10 #the training data will be downsampled in time with i%sample==0
    init_scale = 1.0 #0.1
    learning_rate = 1.0
...
class SmallConfig(Configuration):
    ...
    num_steps = -1
    num_layers = 1
    hidden_size = 5 #dimensionality of cell state and output
    max_epoch = 5
    max_max_epoch = 10
    training_objects = ['dumpster']
    feature_type = 'app' #flow or app
    
class DumpsterConfig(Configuration):
    """train only flying around a dumpster and see if error gets really low"""
    num_steps = -1
    training_objects = ['dumpster']
    validate_objects = ['dumpster']
    test_objects = ['wooden_case']
    
class MediumConfig(Configuration):
    """Medium config, for train small."""
    num_steps = -1
    hidden_size = 16 #dimensionality of cell state and output
    training_objects = ['box', 'dumpster']
    validate_objects = ['box']
    test_objects = ['wooden_case']
...
class BigConfig(Configuration):
    ...
    training_objects = ['box', 'dumpster', 'ragdoll', 'wooden_case']
    validate_objects = ['box', 'dumpster', 'ragdoll', 'wooden_case']
    #training_objects = ['box', 'dumpster', 'ragdoll', 'polaris_ranger', 'wooden_case']
    #validate_objects = ['box', 'dumpster', 'ragdoll', 'polaris_ranger', 'wooden_case']
    test_objects = ['cafe_table']

#def run_epoch(session, model, data, targets, eval_op, merge_op=tf.no_op(), writer=None, verbose=False):
def run_epoch(session, model, data, targets, eval_op, num_steps=1, writer=None, verbose=False):
...
def main(_):
    ...
    training_data, training_labels = pilot_data.prepare_data(config.training_objects, config.sample, config.feature_type)
    validate_data, validate_labels = pilot_data.prepare_data(config.validate_objects, config.sample, config.feature_type)
    test_data, test_labels = pilot_data.prepare_data(config.test_objects, config.sample, config.feature_type)
    ...
    if config.num_steps == -1: config.num_steps = training_data.shape[1]#config.batch_size = 1
