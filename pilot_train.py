...
class SmallConfig(Configuration):
    ...
    num_steps = -1
    num_layers = 2
    hidden_size = 10 #5 #dimensionality of cell state and output
    max_epoch = 5
    max_max_epoch = 10
    dataset = 'generated'
...
class MediumConfig(Configuration):
    ...
    sample = 1
    num_layers = 1
    hidden_size = 5 #5 #dimensionality of cell state and output
    max_epoch = 5
    max_max_epoch = 10
    dataset = 'generated'
    training_objects = ['modeldde']
    validate_objects = ['modeldde']
    test_objects = ['modeldde']
...
def run_epoch(session, model, data, targets, eval_op, num_steps=1, writer=None, verbose=False):
    ...
    if num_steps == data.shape[1]:
        ...
        if verbose:
            print "Accuracy: ",(score/num_steps)
...
def main(_):
    ...
    with tf.Graph().as_default(), tf.Session(config=tf.ConfigProto(log_device_placement=True, allow_soft_placement=True)) as session:
        ...
        print "Loading models: ", (time.time()-start_time)   
